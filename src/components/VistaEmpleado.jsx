...
const VistaEmpleado = () =>{
    ...
    function llenarTablaEmp(){
        const rta = axios.get('http://localhost:3000/api/v1/empleados').then(rest=>{setdatosEmpleados(rest.data.empleados1)})
    }
    useEffect(() => {
        llenarTablaEmp()
 	},[]);
    ...
    function eliminarCampo(id1){
        ...
        axios.delete('http://localhost:3000/api/v1/empleados/'+id1+'')
        llenarTablaEmp()
    }
    ...
    return(
        ...
        <section className="formE row">
            ...
            <button className="btnE btn-outline-primary"onClick={()=>cambioVistaOperacionAgregarEmpleado()}>Agregar</button>
            <button className="btnE btn-outline-primary" onClick={()=>navigate('/home')}>Finalizar</button>
        </section></>      
    )
}
